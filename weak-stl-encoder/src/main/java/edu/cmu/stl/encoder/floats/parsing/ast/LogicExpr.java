...
public class LogicExpr extends AST {
    ...
    @Override public EmitEncodingResult emitEncoding(int time, String rhoVar, boolean isWeak) {
        ...
        if (this.op.equals("and") || this.op.equals("or")) {
            ...
            e.append(EmitEncoding.varDecl("float", rhoVar)); // declare rhoVar
    ...
    @Override public int getSignalLength() {
        int lsl = this.lhs.getSignalLength();
        int rsl = this.rhs.getSignalLength();

        int result = lsl >= rsl ? lsl : rsl;
        // System.out.println("LogicExpr get signal length: " + result + " " + this);
        return result;
    }
}
