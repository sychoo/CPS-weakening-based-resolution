...
public class TempExpr extends AST {
    ...
    @Override public EmitEncodingResult emitEncoding(int time, String rhoVar, boolean isWeak) {
        ...
        e.append(EmitEncoding.varDecl("float", rhoVar)); // declare rhoVar that was passed in
    ...
    @Override public ArrayList<String> getSignalVariables() {
        return this.body.getSignalVariables();
    }

    // i.e. G[0,1](x>1), signal length is 2 = (1 - 0) + 1 + (1) - 1
    // i.e. G[1,3](x>1), signal length is 4 = (3 - 0) + 1 + (1) - 1
    // i.e. G[0,1](F[0,5]), signal length is 7 = (1 - 0) + 1 + (5 - 0) + 1 - 1
    // signal length is only relevant to upperbound 
    @Override public int getSignalLength() {
        return ((Int) this.upperBound).toInt() + 1 + (this.body.getSignalLength() - 1);
    }
}
