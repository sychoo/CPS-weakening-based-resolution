...
//////////////////////////////////
// TURN ANTLR PARSE TREE TO AST //
//////////////////////////////////

// modified purpose: generate AST from STL file
// module requirement by ANTLR parser 
//
...
import edu.cmu.stl.encoder.parser.STLBaseListener;
...
// import edu.cmu.stl.encoder.ints.encoder.ast.*;
import edu.cmu.stl.encoder.floats.parsing.ast.*;
import edu.cmu.stl.encoder.floats.parsing.ast.Float;

import edu.cmu.stl.encoder.floats.parsing.EmitEncoding;

public class CompilationListener extends STLBaseListener {
    STLParser parser;
    Encoding encoding;
    Encoding debugEncoding;
    Signal signal;

    ArrayList<AST> stlASTList = null;
    ...
    // after the parse tree walker, the AST is stored in this variable arithStack and compStack
    public CompilationListener (STLParser parser) {
        ...
        this.parser = parser;
        EmitEncodingResult er = EmitEncoding.prelude();
        this.encoding.append(er.getEncoding());
        this.debugEncoding.append(er.getDebugEncoding());

        this.stlASTList = new ArrayList<AST>();
    }


    // public CompilationListener(STLParser parser, Signal signal) {
    //     arithStack = new Stack<AST>();
    //     compStack = new Stack<AST>();
    ...
    // public CompilationListener(STLParser parser, Signal signal, String envString) {
    //     this(parser, signal);
    ...
    @Override
    public void enterProg(STLParser.ProgContext ctx) {
    ...
    @Override
    public void exitProg(STLParser.ProgContext ctx) {
    ...
    @Override
    public void enterStat(STLParser.StatContext ctx) {
    ...
    // exit statements
    @Override
    public void exitStat(STLParser.StatContext ctx) {
        ...
        if (this.compStack.size() == 1) {
            ...
            // System.out.println(expr); // test, print out the final AST.

            this.stlASTList.add(expr); // set the AST to the globally variable.
            ...
            if (EmitEncoding.statWeakenFlag) {
                ...
                for (String paramVar: EmitEncoding.paramList) {
                    this.encoding.append(EmitEncoding.varDecl("float", paramVar));
                }
    ...
    // @Override
    // public void exitStlNotExpr(STLParser.StlNotExprContext ctx) {
    //     AST stlExpr = this.compStack.pop();
    //     this.compStack.push(new LogicExpr("not", null, stlExpr));
    // }
    
    @Override
    public void exitStlAndExpr(STLParser.StlAndExprContext ctx) {
    ...
    @Override
    public void exitStlOrExpr(STLParser.StlOrExprContext ctx) {
        AST right = this.compStack.pop();
        AST left  = this.compStack.pop();
    ...
    @Override
    public void exitStlImpExpr(STLParser.StlImpExprContext ctx) {
        AST right = this.compStack.pop();
        AST left  = this.compStack.pop();
    ...
    @Override
    public void exitStlIffExpr(STLParser.StlIffExprContext ctx) {
        AST right = this.compStack.pop();
        AST left  = this.compStack.pop();
    ...
    // @Override
    // public void exitStl(STLParser.StlContext ctx) {
    ...
    @Override
    public void exitGlobally(STLParser.GloballyContext ctx) {
    ...
    @Override
    public void exitEventually(STLParser.EventuallyContext ctx) {
            // construct TempExpr AST
            AST expr = this.compStack.pop();
            AST tempExpr = new TempExpr(ctx.op.getText(),
                                        new Int(ctx.timeBound().INT(0).getText()),
    ...
    // public AST exitWeakTemp(STLParser.WeakGloballyContext globallyCtx, STLParser.WeakEventuallyContext eventuallyCtx) {

    // }

    @Override
    public void exitWeakGlobally(STLParser.WeakGloballyContext ctx) {
    ...
    @Override
    public void exitWeakEventually(STLParser.WeakEventuallyContext ctx) {
    ...
    @Override
    public void exitAndExpr(STLParser.AndExprContext ctx) {
        AST left  = this.compStack.pop();
        AST right = this.compStack.pop();
        AST logicAST = new LogicExpr("and", left, right);
        this.compStack.push(logicAST);
    ...
    @Override
    public void exitOrExpr(STLParser.OrExprContext ctx) {
        AST right = this.compStack.pop();
        AST left  = this.compStack.pop();
        AST logicAST = new LogicExpr("or", left, right);
        this.compStack.push(logicAST);
    ...
    @Override
    public void exitNotExpr(STLParser.NotExprContext ctx) {
        AST right = this.compStack.pop();
        AST left = null; // set left to null since only 1 arg is passed to not
        AST logicAST = new LogicExpr("not", left, right);
        this.compStack.push(logicAST);
    ...
    @Override
    public void exitImpExpr(STLParser.ImpExprContext ctx) {
        AST right = this.compStack.pop();
        AST left  = this.compStack.pop();
        AST logicAST = new LogicExpr("imp", left, right);
        this.compStack.push(logicAST);
    ...
    @Override
    public void exitIffExpr(STLParser.IffExprContext ctx) {
        AST right = this.compStack.pop();
        AST left  = this.compStack.pop();
        AST logicAST = new LogicExpr("iff", left, right);
        this.compStack.push(logicAST);
    ...
    @Override 
    public void exitInt(STLParser.IntContext ctx) { 
        AST intAST = new Int(ctx.getText());
        this.arithStack.push(intAST);
    }

    @Override 
    public void exitFloat(STLParser.FloatContext ctx) { 
        AST floatAST = new Float(ctx.getText());
        this.arithStack.push(floatAST);
    }

    @Override 
    public void exitParens(STLParser.ParensContext ctx) { 
        // AST parensAST = new Parens(ctx.arithExpr()
    }

    // walk through and copy the arith expr via stack without evaluating them
    @Override 
    public void exitMulDivExpr(STLParser.MulDivExprContext ctx) { 
    ...
    @Override 
    public void exitAddSubExpr(STLParser.AddSubExprContext ctx) { 
        AST right = this.arithStack.pop();
        AST left  = this.arithStack.pop();
    ...
    @Override
    public void exitNormalCompExpr(STLParser.NormalCompExprContext ctx) {
        AST right = this.arithStack.pop();
        AST left  = this.arithStack.pop();
    ...
    // push the normalized form of compExpr onto the stack
    @Override
    public void exitWeakCompExpr(STLParser.WeakCompExprContext ctx) {
        ...
        AST param_ub = new Float(ctx.FLOAT().getText());
    ...
    public ArrayList<AST> getASTList() {
        // System.out.println("getAST(): " + this.stlAST);
        if (this.stlASTList == null) {
            throw new RuntimeException("AST is not set");
        } else {
            return this.stlASTList;
        }
    }
}
