...
//
// note that atomic proposition (AP) represent a single logical expr (x > 1) that is evaluated to true or false.
// thus it has now been changed logicExpr

stl: 	'(' stl ')'				   # StlParenExpr
...
compExpr: arithExpr op = ( '=' | '>' | '<' | '>=' | '<=' ) arithExpr # NormalCompExpr
		| '<<' arithExpr op = ('>' | '<' | '>=' | '<=') arithExpr '>>' '(' ID ',' INT ')' # WeakCompExpr
		;
...
timeBound: 			'[' INT ',' INT ']' ; // normal time bound														
weakLeftTimeBound: 	'[' '<<' INT '>>' '(' ID ',' INT ')' ',' INT ']';	// weaken left side time bound							
weakRightTimeBound:	'[' INT ',' '<<' INT '>>' '(' ID ',' INT ')' ']';   // weaken right side time bound
weakBothTimeBound: 	'[' '<<' INT '>>' '(' ID ',' INT ')' ',' '<<' INT '>>' '(' ID ',' INT ')' ']'; // weaken both sides of the time bound
...
// language constructs
ID: [a-z][a-zA-Z0-9_]*; // match identifiers lowerCase (UpperCase + Number)*
