...
osascript -e 'tell app "Terminal"
    do script "
        pushd $PX4_DIR/mavsdk-server/ 
        gtimeout $RUNPX4_TIME ./mavsdk_server_macos -p 50052 udp://:14541
...
osascript -e 'tell app "Terminal"
    do script "\
        ...
        ./Tools/sitl_multiple_run.sh 2
        gtimeout $RUNPX4_TIME ./Tools/jmavsim_run.sh -p 4560 -l
        popd
        logout"
end tell'
popd
...
osascript -e 'tell app "Terminal"
    do script "
        ...
        pushd $PX4_DIR/PX4-Autopilot
        gtimeout $RUNPX4_TIME ./Tools/jmavsim_run.sh -p 4561 -l
        logout
        "
end tell'
popd
...
osascript -e 'tell app "Terminal"
    ...
    pushd $PX4_DIR/PX4-Autopilot/PX4Scripts
    # gtimeout $RUNPX4_TIME 
    ./run_airsim_sitl.sh 1; 
    # logout 
	# connect px4 shell to instance 1
    param set MIS_TAKEOFF_ALT 10.0 # takeoff altitude
    param set COM_DL_LOSS_T 300 # avoid data link timeouts
        param set MIS_TAKEOFF_ALT 10.0 # takeoff altitude
        param set COM_RCL_EXCEPT 4 # disable joystick (prevent failsafe warning)
